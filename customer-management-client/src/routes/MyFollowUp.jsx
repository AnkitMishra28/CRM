...
const MyFollowUp = () => {
  ...
  const fetchFollowUps = async () => {
    const res = await axios.get(`http://localhost:3000/myfollowUp/${user?.email}`, { withCredentials: true });
    return res.data;
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:3000/api/followups/${id}`, { withCredentials: true });
          refetch();
          ...
          <tbody>
            {followups.map((item, index) => (
              <tr key={item._id} className="hover:bg-gray-50 transition">
                ...
                <td className="py-3 px-4 border">{item.followUpDate}</td>
                <td className="py-3 px-4 border">{item.time}</td>
